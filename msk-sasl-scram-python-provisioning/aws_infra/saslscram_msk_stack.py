...
class MskWithSaslScramAuthStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, vpc: ec2.Vpc, cluster_name='SaslScram', brokers=2, **kwargs):
        ...
        # Create a CMK for MSK SASL/SCRAM secret. A secret created with the default AWS KMS key cannot be used with an Amazon MSK cluster. So, we need to create a CMK
        cmk = kms.Key(self, 'MskSaslScramCmk') # Customer Managed Key for MSK SASL/SCRAM secret
        # The secret is generated with a random password, which is encrypted with the CMK
        secret = secrets_mgr.Secret(
            self,
            'MskSaslScramSecret',
            encryption_key=cmk,
            secret_name='AmazonMSK_SaslScram', # Has to start with AmazonMSK_
        ...
        msk.CfnBatchScramSecret(self, 'SaslScramSecretAttachment', cluster_arn=kafka.attr_arn, secret_arn_list=[secret.secret_arn]) #can have only 10 secrets in the list, use multiple such lines if needed. 
        # Because, according to AWS Docs: You can associate up to 10 secrets with a cluster at a time using the BatchAssociateScramSecret operation.
